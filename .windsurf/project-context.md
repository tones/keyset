...
- **`playChord`** (`src/lib/playChord.ts`) — Plays a chord from MIDI notes using Tone.js `PolySynth` with a triangle wave. Lazy-initializes the synth on first use. Calls `Tone.start()` to handle browser autoplay policy.
