...
- `src/app/song/[id]/actions.ts` — `updateYoutubeUrl`, `updateSongTitle`, `reorderKeySets`, `createKeySet`, `duplicateKeySet` (copies key set with all key presses, inserts after original), `deleteKeySet`, `toggleKeyPress`, `shiftNotes` (moves all notes by any delta, clamped to 0–127), `shiftOctave` (wrapper for shiftNotes ±12), `updateKeySetType` (toggles between "chord" and "flourish")
