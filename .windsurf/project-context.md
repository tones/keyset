...
- **`pianoLayout`** (`src/lib/pianoLayout.ts`) — Shared piano key layout math used by both `PianoKeyboard` and `CommonToneLines`. Exports `isBlackKey`, `getNoteName`, `buildKeyLayout`, `blackKeyLeftPct`, `keyCenterPct`, and `BLACK_KEY_BIAS`. Single source of truth for key positioning.
