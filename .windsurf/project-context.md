...
- **Redeploy:** `npm run deploy` from the project root (passes `GIT_SHA` build arg automatically). The short git commit hash is displayed next to "Keysets" on the home page via `NEXT_PUBLIC_GIT_SHA` (set in `next.config.ts`).
