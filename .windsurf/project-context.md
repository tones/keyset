...
- **`scales`** (`src/lib/scales.ts`) — Scale definitions and utilities for In Key mode. Defines 12 roots (C through B) and 8 modes (major, minor, dorian, phrygian, lydian, mixolydian, aeolian, locrian) with semitone intervals. Exports `ROOTS`, `MODE_NAMES`, `MODES`, `parseSongKey`, `formatSongKey`, `getScalePitchClasses`, `isNoteInKey`, `getTriadPitchClasses`, `getTriadName` (chord name for a degree, e.g. "Dm"), `getTriadQuality` (major/minor/diminished for a degree).
