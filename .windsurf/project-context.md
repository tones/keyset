...
- **`SongView`** (`src/components/SongView.tsx`) — Client wrapper that owns all draft state: key sets AND analysis. Renders: (1) a top row with subtle "‹ Keysets" breadcrumb on the left and minimal save bar ("Edited" + dot, invisible when clean) on the right; (2) album art (160px, rounded, shadow) on the left with title/YouTube below it, toggle switches stacked vertically in the upper right; (3) `SortableKeySetList`; (4) `SongAnalysis`. Two toggle switches: **Compact** (dark) and **Guides** (yellow). Analysis is part of deferred save — analyzing, re-analyzing, or deleting analysis only updates local state. The `saveKeySets` action persists both key sets and analysis atomically. Reset reverts both key sets and analysis to server state. `buildChordDetail()` computes chord descriptions from live browser keySets (not DB), so analyze and chat always use current edits. Dirty detection uses `useRef` snapshot comparison over both key sets and analysis. Also provides: `beforeunload` warning, confirmation dialog on Reset, Cmd/Ctrl+S shortcut.
